...
export default function ExpandedSurvey() {
  ...
  const handleInfoSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const savedSurveyData = JSON.parse(localStorage.getItem('expandedSurvey') || '{}');
  ...
  const handleNext = async (answerOverride?: string) => {
    ...
    if (answerToUse) {
      ...
      if (currentQuestionIndex < expandedQuestions.length - 1) {
        ...
        // All questions answered - save responses and move to academic background form
        const surveyData = {
          userInfo,
  ...
  if (step === 'academicBackground') {
    ...
    const storageKey = 'expandedSurvey';
